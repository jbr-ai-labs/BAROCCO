...
class ReplayMemoryLm(ReplayMemory):
    ...
    def push(self, global_state):
        batch = dict()
        batch['agents'] = deepcopy(self.agents)
        batch['global_state'] = global_state
        self.append_batch([batch])
        self.clear_agents()
    ...
    def select_batch(self):
        if self.config.REPLAY_LM:
            ...
            return np.arange(len(self) - 1 - self.nSteps), None
...
class ReplayMemoryLmMaster:  # ReplayMemoryLm per worker
    def __init__(self, args, config):
        self.args, self.config = args, config
        self.memories = [ReplayMemoryLm(self.config) for _ in range(args.nEnvs)]
    ...
    def sample(self):
        ...
        for _ in range(len(ints)):
            ...
            if len(self.memories[i]) > 0:
                ret = self.memories[i].sample(), i
                if not self.config.REPLAY_LM: self.memories[i].reset()
                return ret
            ints.remove(i)
        return None, None
    ...
    def update_priorities(self, idx, priorities, i):
        return self.memories[i].update_priorities(idx, priorities)

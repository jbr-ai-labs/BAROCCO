...
class MapEnv:
    ...
    def __init__(self, ascii_map, num_agents=1, render=True, color_map=None, norm=False):
        """
    ...
    def step(self, actions):
        ...
        for agent in self.agents.values():
            ...
            rgb_arr = self.map_to_colors(agent.get_state(), self.color_map, norm=self.norm)
            rgb_arr = self.rotate_view(agent.orientation, rgb_arr)
            observations[agent.agent_id] = rgb_arr
            info['apples'][agent.agent_id] = agent.compute_apples()
    ...
    def get_map_with_agents(self):
        ...
        for agent_id, agent in self.agents.items():
            char_id = dec_to_base(int(agent_id.lstrip('agent-')) + 1)
    ...
    def map_to_colors(self, map=None, color_map=None, norm=False):

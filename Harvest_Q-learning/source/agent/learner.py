...
class Model:
    ...
    def loadLmFrom(self, states):
        states, state_mixer = states
        states = [self.convertStateDict(state) for state in states]
        [lm.load_state_dict(state) for lm, state in zip(self.social, states)]
        self.mixer.load_state_dict(self.convertStateDict(state_mixer))
    ...
    def sendTargetAnns(self):
        states = [ann.state_dict() for ann in self.targetAnns]
        states = [self.convertStateDict(state, device='cpu') for state in states]
        return states
    ...
    def sendLm(self):
        ...
        states = [self.convertStateDict(state, device='cpu') for state in states]
        state_mixer = self.convertStateDict(self.mixer.state_dict(), device='cpu')
        return [states, state_mixer]
